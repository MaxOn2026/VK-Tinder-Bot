...
def get_user_statistics(user_id: int) -> dict:
    ...
    with db_manager.get_session() as session:
        ...
        likes_count = (
            session.query(func.count(UserInteraction.id))
            .filter(
                UserInteraction.user_id == user.id, UserInteraction.action == "like"
            )
            .scalar()
        )
        ...
        blocks_count = (
            session.query(func.count(UserInteraction.id))
            .filter(
                UserInteraction.user_id == user.id, UserInteraction.action == "block"
            )
            .scalar()
        )
